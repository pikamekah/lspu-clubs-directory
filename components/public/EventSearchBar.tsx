...
type Props = {
  ...
  searchSuggestions: EventSuggestion[];
  applySearch: (value: string) => void;
  openSuggestion: (suggestion: EventSuggestion) => void;
  clearSearch: () => void;
};
    ...
    <div className="relative mb-5">
      <div className="bg-white rounded-[26px] sm:rounded-full p-2 flex flex-col sm:flex-row sm:items-center gap-2 shadow-xl border border-cyan-100">
        ...
        <input
          ...
          placeholder="Search events by title, organization, category, or venue..."
          title="Search event"
        ...
        <div className="absolute left-0 right-0 top-[118px] sm:top-[66px] bg-white rounded-[28px] shadow-2xl border border-cyan-100 overflow-hidden z-30">
          {searchSuggestions.map((suggestion, index) => (
            ...
            >
              ...
              <span
                className={`text-xs px-3 py-1 rounded-full h-fit font-bold whitespace-nowrap ${getCategoryStyle(
                  suggestion.category
                )}`}
              >
                {suggestion.category}
